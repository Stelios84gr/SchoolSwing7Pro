...
//				cities.forEach(city -> cityComboBox.addItem(city));
		...
		setIconImage(Toolkit.getDefaultToolkit().getImage(TeacherView.class.getResource("/images/eduv2.png")));
		...
		JLabel firstLastName = new JLabel("ΣΤΥΛΙΑΝΟΣ ΦΡΥΔΑΚΗΣ");
		firstLastName.setHorizontalAlignment(SwingConstants.CENTER);
...
//			ps.setInt(1,  id);
			...
			if (rs.next()) {
				...
				phoneNumText.setText(rs.getString("phonenum"));
...
//				cityComboBox.setSelectedIndex(rs.getInt("city_id") - 1);
				...
				City selectedCity = cities.stream().filter(city -> city.getId() == cityIdFromDB).findFirst().orElse(null);
