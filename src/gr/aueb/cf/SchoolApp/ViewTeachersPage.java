...
//		String sql = "SELECT id, firstname, lastname FROM teachers WHERE lastname LIKE ?";
		...
		try (PreparedStatement ps = conn.prepareStatement(sql)) {
			ps.setString(1, lastnameText.getText().trim() + "%");	// το % χρησιμοποιείται ως wildcard για να τρέξει το LIKE
...
//			ps.setInt(1,  id);
			...
			if (answer == JOptionPane.YES_OPTION) {
				...
				JOptionPane.showMessageDialog(null, rowsAffected + " γραμμή/ές διαγράφηκαν", " Διαγραφή", JOptionPane.INFORMATION_MESSAGE);
			} else {
				return;
			}
